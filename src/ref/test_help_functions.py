...
def split_test_train_whole_train_split(data=load_iris(), test_size=0.1, balance_size=0.3):
    x_train, x_test, y_train, y_test = train_test_split(data.get("data"), data.get("target"), test_size=test_size)
...
def test_k_federated_algorithm_for_each_balance_step_iterative(data=load_iris(),
    ...
    score_whole = classifier_whole.score(dic.get("test").get("X"), dic.get("test").get("y"))
    print("whole")
    print(score_whole)
    ...
    for i in range(0, len(db_lists)):
        ...
        score_federated = classifier_federated.score(dic.get("test").get("X"), dic.get("test").get("y"))
        #print(score_federated)
        res.append(score_federated)
        print(str(round(balance_list[i],1))+ " "+ str(round(((len_data-len_test)*balance_list[i]),0)) + " "+ str(round(score_federated,2)) + " " + str(time_list[i]))
